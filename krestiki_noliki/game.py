...
class Game:
    ...
    def __make_player(self, player_symbol: str, player_factory: PlayerFactory) -> Player:
        while True:
            ...
            if player_type == str(PlayerFactory.HUMAN_PLAYER):
                return player_factory.create(PlayerFactory.HUMAN_PLAYER)
            elif player_type == str(PlayerFactory.COMPUTER_PLAYER):
                return player_factory.create(PlayerFactory.COMPUTER_PLAYER)
            else:
    ...
    def play(self):
        ...
        for i in range(9):
            ...
            print(f"Ходит игрок {player_symbol}")
